...
def meta_data(scripts): 
    ct_script = [script for script in scripts if "captionTracks" in str(script)]
    if len(ct_script) == 0:
        return []
...
def detect_problem(scripts):
    error_script = [script for script in scripts if "playerErrorMessageRenderer" in str(script)]
...
def sort_through_md(md, langCode = None, lang = None, priority = None):
    ...
    if langCode is None and lang is None:
        ...
        if len(set(langCodes)) != 1:
            raise Exception("Captions in more than one language available. \n Please specify a language or a languageCode")
        else:
    ...
    if lang is not None:
        items = [item for item in md if lang.lower() in item['name']['simpleText'].lower()]  
    
    if len(items) == 0:
        return None
    
    nonauto_items = [item for item in items if '(auto-generated)' not in item['name']['simpleText']]
    auto_items = [item for item in items if '(auto-generated)' in item['name']['simpleText']]
...
def cc(videoId, langCode = None, lang = None, meta = False):
    ...
    if lang is not None and langCode is not None:
        raise Exception("Specify either a language or a languageCode, not both.")
    ...
    if len(md) == 0:
        try:
            ...
            if meta:
                return {'not_available' : 'No CC available for this video in any language.'}
            else:
    ...
    if preferred_items is None:
        if meta:
            return {'not_available' : 'No CC available for this video in this language.'}
        else:
